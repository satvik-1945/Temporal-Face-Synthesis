"""SimSwap 256 - higher quality, 256px resolution (neuralchen)."""
...
# ImageNet normalization for SimSwap
SIMSWAP_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
SIMSWAP_STD = np.array([0.229, 0.224, 0.225], dtype=np.float32)


def _ensure_models(model_dir: Path) -> tuple[Path, Path]:
    """Download simswap_256.onnx and crossface_simswap.onnx if needed."""
    model_dir.mkdir(parents=True, exist_ok=True)
    simswap_path = model_dir / "simswap_256.onnx"
    crossface_path = model_dir / "crossface_simswap.onnx"

    try:
        from huggingface_hub import hf_hub_download

        if not simswap_path.exists():
            hf_hub_download(
        ...
        raise RuntimeError(
            f"Could not download SimSwap models. Error: {e}"
        ) from e

    return simswap_path, crossface_path


class SimSwapSwapper(SwapperBase):
    name = "simswap"

    def __init__(self):
        self._size = (256, 256)
        self._mean = SIMSWAP_MEAN
        self._std = SIMSWAP_STD
    ...
    def load(self, model_dir: Optional[Path] = None) -> tuple[Any, Any]:
        ...
        simswap_path, crossface_path = _ensure_models(model_dir)
        ...
        try:
            face_swapper = onnxruntime.InferenceSession(
                str(simswap_path), providers=providers
            )
            ...
            face_swapper = onnxruntime.InferenceSession(
                str(simswap_path), providers=["CPUExecutionProvider"]
            )
            embedding_converter = onnxruntime.InferenceSession(
    ...
    def swap(
        ...
        source_face = source_faces[si]
        target_face = target_faces[ti]

        # Convert source embedding (buffalo_l 512 -> simswap format)
        ...
        # Prepare target: BGR->RGB, /255, ImageNet normalize
